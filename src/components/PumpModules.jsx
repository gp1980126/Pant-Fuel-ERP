  ...
  return <div className="content"><section className="panel" style={{marginBottom:12}}><div className="form"><label>Financial Year<select value={selectedFY} onChange={e=>setSelectedFY(e.target.value)}>{FINANCIAL_YEARS.map(y=><option key={y.value} value={y.value}>{y.label}</option>)}</select></label></div><div style={{marginTop:6,color:"#64748b"}}>Selected: {fy.start} to {fy.end}</div></section>
    ...
    <section className="panel" style={{marginTop:18,border:'2px solid #16a34a'}}>
      ...
      <div className="form">
        ...
        <Field label="Rate / L"><input type="number" min="0" step="0.01" value={cashSale.rate} onChange={e=>{const rate=e.target.value;const qty=n(cashSale.packQty)>0&&n(cashSale.packSize)>0?rupee(n(cashSale.packQty)*n(cashSale.packSize)):n(cashSale.qty);setCashSale(x=>({...x,rate,amount:n(rate)>0&&qty>0?rupee(qty*n(rate)):""}));}}/></Field>
        <Field label="Amount (Auto)"><input type="number" min="0" step="0.01" value={n(cashSale.qty)>0&&n(cashSale.rate)>0?rupee(n(cashSale.qty)*n(cashSale.rate)):""} readOnly placeholder="Qty × Rate auto"/></Field>
